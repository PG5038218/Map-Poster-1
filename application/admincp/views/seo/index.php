                                    ...
                                    <?php if (count($settings) > 0) { ?>
                                        <?php for ($i = 0; $i < count($settings); $i++) { ?>   
                                            <tr class="gradeX">
                                                ...
                                                <td><?php echo htmlspecialchars($settings[$i]['field_value']); ?></td>
                                                <td class="text-center">
                                                    <?php if ($settings[$i]['status'] == 'Enable') : ?>
                                                    <a href="<?php echo base_url('seo/change_status/'.  base64_encode($settings[$i]['seo_id'])); ?>" data-toggle="modal" data-target="#change-status" class="btn btn-success btn-xs" title="Disable"><?php echo ($settings[$i]['status']); ?></a>
                                                    <?php else: ?>
                                                    <a href="<?php echo base_url('seo/change_status/'.  base64_encode($settings[$i]['seo_id'])); ?>" data-toggle="modal" data-target="#change-status" class="btn btn-danger btn-xs" title="Enable"><?php echo ($settings[$i]['status']); ?></a>
                                                    <?php endif; ?>
                                                </td>
                                                <td class="text-center">
                                                    <a href="<?php echo base_url('seo/update/'.  base64_encode($settings[$i]['seo_id'])); ?>" data-toggle="modal" data-target="#edit-modal" title="Edit"> <i class="glyphicon glyphicon-edit"></i></a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    <?php } else { ?> 
